...
__kps1_init() {
  ...
  local namespace=$(yq '.contexts[0].context.namespace // "default"' $HOME/.kube/config)
...
__kps1_switch_context() {
  local kubeconfig=$(ls -p $HOME/.kube/config.d | grep -v / | fzf --no-sort)
  if [[ -z "$kubeconfig" ]]; then
    return
  fi
  unalias __kps1
  alias __kps1="kubectl --kubeconfig=$HOME/.kube/config.d/$kubeconfig"

  local context=$(yq '.contexts[0].name' $HOME/.kube/config.d/$kubeconfig)
...
__kps1_switch_namespace() {
  ...
  (__kps1 config set-context --current --namespace="${ns}" > /dev/null &)
